...
/// 描述一个视频的来源。
///
/// * [NetworkVideoSource] ─ 网络地址（`http(s)://...`）。
/// * [FileVideoSource]    ─ 本地绝对路径或 `file://` URI。
/// * [AssetVideoSource]   ─ Flutter assets，由插件自动抽取到临时目录后以 `file://` 交给原生解码器。
///
/// A sealed description of a video source.
///
/// * [NetworkVideoSource] – HTTP(S) URL.
/// * [FileVideoSource]    – Local absolute path or `file://` URI.
/// * [AssetVideoSource]   – Flutter asset; the plugin extracts it to the temp
///   directory and hands the `file://` URI to the native decoder.
@immutable
sealed class VideoSource {
  ...
  /// 将来源解析为原生解码器可直接消费的 URL。
  ///
  /// * 网络：原样返回 URL
  /// * 文件：规范化为 `file:///absolute/path`
  /// * 资源：懒抽取到缓存目录并返回 `file://` URI
  ///
  /// Resolve this source into a URL the native decoder can consume directly.
...
/// 本地文件来源（绝对路径或 `file://` URI）。
/// Local file video source (absolute path or `file://` URI).
...
/// Flutter asset video source; extracted to the temp directory on first use.
